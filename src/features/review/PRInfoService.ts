...
/** gh CLI JSON response types (internal) */
...
export class PRInfoService {
  ...
  async getPRFiles(prNumber: number, targetRepo?: string): Promise<PRFileChange[]> {
    try {
      ...
      const cwd = this.getWorkspaceRoot();
      const repoOption = targetRepo ? `--repo ${targetRepo}` : '';
      const { stdout } = await exec(`gh pr view ${prNumber} ${repoOption} --json files`, { cwd });
      ...
      return data.files.map((file: GHFileResponse) => ({
        path: file.path,
  ...
  async getPRCommits(prNumber: number, targetRepo?: string): Promise<PRCommit[]> {
    try {
      ...
      const cwd = this.getWorkspaceRoot();
      const repoOption = targetRepo ? `--repo ${targetRepo}` : '';
      const { stdout } = await exec(`gh pr view ${prNumber} ${repoOption} --json commits`, { cwd });
      ...
      return data.commits.map((commit: GHCommitResponse) => ({
        sha: commit.oid,
  ...
  async getFileDiff(
    ...
    try {
      ...
      const cwd = this.getWorkspaceRoot();
      const repoOption = targetRepo ? `--repo ${targetRepo}` : '';
      const { stdout } = await exec(`gh pr diff ${prNumber} ${repoOption} -- "${filePath}"`, {
        cwd,
      });
      return stdout;
    } catch (error) {
  ...
  async getPRComments(prNumber: number, targetRepo?: string): Promise<PRReviewComment[]> {
    try {
      ...
      return comments.map((comment: GHCommentResponse) => ({
        id: comment.id,
  ...
  filterTranslationFiles(files: PRFileChange[], lang: string = 'all'): PRFileChange[] {
    ...
    return files.filter((file) => {
      const filePath = file.path.toLowerCase();
      return filePath.includes(`content/${lang}/`) || filePath.includes(`i18n/${lang}/`);
    });
  }
  ...
  getReviewableFiles(files: PRFileChange[], lang: string = 'all'): PRFileChange[] {
    return this.filterMarkdownFiles(this.filterTranslationFiles(files, lang));
  }
  ...
  private _normalizeStatus(status: string): 'added' | 'modified' | 'removed' | 'renamed' {
    ...
    if (lowerStatus.includes('add')) {
      return 'added';
    }
    if (lowerStatus.includes('modif') || lowerStatus.includes('change')) {
      return 'modified';
    }
    if (lowerStatus.includes('delet') || lowerStatus.includes('remov')) {
      return 'removed';
    }
    if (lowerStatus.includes('renam')) {
      return 'renamed';
    }
  ...
  async checkoutPR(prNumber: number, prTitle: string, targetRepo?: string): Promise<void> {
    try {
      ...
      if (branchExists) {
        ...
        console.log(
          `Branch ${branchName} already exists, checking out and pulling latest changes...`
        );
  ...
  async listRecentPRs(
    ...
    try {
      ...
      return data.map((pr: GHPRListResponse) => ({
        number: pr.number,
        title: pr.title,
