...
import { i18n, LANGUAGE_NAMES, SUPPORTED_LANGUAGES, LANGUAGE_OPTIONS } from '../i18n';
import {
  extractLanguageCode as _extractLanguageCode,
  getOriginalEnglishPath,
...
export class TranslationUtils {
   ...
   */
  async compareLineCounts(
    originalPath: string,
    translationPath: string
  ): Promise<FileTranslationProgress | null> {
    try {
      ...
      const [originalContent, translationContent] = await this.readBothFiles(
        originalPath,
        translationPath
      );
      const [originalLines, translationLines] = this.calculateLineCounts(
        originalContent,
        translationContent
      );
  ...
  private async openFilesInSplitView(originalPath: string, translationPath: string): Promise<void> {
    ...
    await vscode.commands.executeCommand('vscode.open', originalUri, {
      viewColumn: vscode.ViewColumn.One,
    });

    const translationUri = vscode.Uri.file(translationPath);
    await vscode.commands.executeCommand('vscode.open', translationUri, {
  ...
  private scrollEditorsToTop(): void {
    setTimeout(() => {
      ...
      editors.forEach((editor) => {
        ...
        editor.revealRange(
          new vscode.Range(topPosition, topPosition),
  ...
  private async checkFileCreationPreconditions(
    originalPath: string,
    translationPath: string
  ): Promise<boolean> {
  ...
  private async checkBothFilesExist(
    originalPath: string,
    translationPath: string
  ): Promise<boolean> {
    const originalExists = await this.fileExists(originalPath);
  ...
  private calculateLineCounts(
    originalContent: Uint8Array,
    translationContent: Uint8Array
  ): [number, number] {
